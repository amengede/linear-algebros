...
int main() {
	...
	mat4 projection_transform = linalgMakePerspectiveProjection(45.0f, aspectRatio, 0.1f, 10.0f);
	glUniformMatrix4fv(glGetUniformLocation(shader, "projection"), 1, GL_FALSE, projection_transform.data);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//update transform
		float angle = static_cast<float>(10 * glfwGetTime());
		mat4 model_transform = linalgMakeIdentity4();
		vec3 position = linalgMakeVec3(0.0f, 0.0f, -3.0f);
		model_transform = linalgMulMat4Mat4(model_transform, linalgMakeXRotation(angle));
		model_transform = linalgMulMat4Mat4(model_transform, linalgMakeYRotation(2.0f * angle));
		model_transform = linalgMulMat4Mat4(model_transform, linalgMakeTranslation(position));
		glUniformMatrix4fv(glGetUniformLocation(shader, "model"), 1, GL_FALSE, model_transform.data);
